...
df_flux_entrant_recyclage = pd.read_csv('db_csv/db_flux_entrant_recyclage.csv',header=0).set_index(['Region','TU'])
liste_annees = [i for i in range(1901,2101)]

print(liste_annees)

somme_france = df_flux_entrant_recyclage.loc['France'].sum()
somme_world = df_flux_entrant_recyclage.loc['World'].sum()
...
fig.suptitle('Flux entrant en nombre de pneus en prenant (recyclage pris en compte)')
